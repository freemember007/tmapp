...
if(Ti.App.Properties.getString("avatar")== null){
...
function hideNavBar(e){
	if(e.contentOffset.y - offset > 10 && e.contentSize.height>Ti.Platform.displayCaps.platformHeight && isHide == false){
		$.top.animate({top:-47});
		$.table.animate({top:0});
	...
	util.send('api/fetchBlog', {email: Ti.App.Properties.getString("email"), password: Ti.App.Properties.getString("password"), offset:fetchOffset}, function(res){
		...
		if(data.type == "success"){
			...
			for(key in items){
				...
				var section = Alloy.createController('blogSection', arg).getView();
				$.table.appendSection(section,{animated:false});
			};
			$.table.deleteSection($.table.data.length-Object.keys(items).length-1,{animated:false});
			$.table.scrollToIndex(lastRow-10,{position:Ti.UI.iPhone.TableViewScrollPosition.BOTTOM});
			fetchOffset += 10;
		}else if(data.type == "fail"){
			alert('用户名或密码错误！');
		}else{
...
$.table.addEventListener('scroll',function(e){
	if ((!updating) && e.contentSize.height>Ti.Platform.displayCaps.platformHeight*5 && (e.contentOffset.y + e.size.height + 100 > e.contentSize.height)) {
        beginUpdate();
    }
});